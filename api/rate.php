...
class Rate{
    ...
    public $coef_SL = 0.025; // коэфициент линии SL под текущим курсом

    public $coef_TP = 0.025; // коэфициент линии TP над текущим курсом

    public $current; // текущий курс
    public $sl; // курс линии SL под текущим курсом
    public $tp; // курс линии tp над текущим курсом
    ...
    function __construct($size, $delay, $v){
        ...
        if ($this->values[0]>=0)
            ...
            $this->sign = -1;
        $this->sl = $v - $v*$this->coef_SL;
        $this->tp = $v + $v*$this->coef_TP;
        return;
    }
    ...
    public function refresh($value){
        ...
        if ($this->values[0]<$this->values[1])
            $this->sign = -1;
        $this->current = $value;
        if ($this->current - $this->sl > $this->coef_SL * $this->current)
            $this->sl = $this->current - $this->coef_SL * $this->current;
        if ($this->tp - $this->current > $this->coef_TP * $this->current)
            $this->tp = $this->current + $this->coef_TP * $this->current;
        return;
    }

    
    public function refresh_sltp(){
        $this->sl = $this->current - $this->coef_SL * $this->current;
        $this->tp = $this->current + $this->coef_TP * $this->current;
        return;
    }
